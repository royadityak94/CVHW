...
if __name__ == '__main__':
    sift = cv2.xfeatures2d.SIFT_create()
    img1 = cv2.imread("../data/stitching/hill1.jpg")
    img1 = cv2.cvtColor(img1, cv2.COLOR_BGR2GRAY)
    img2 = cv2.imread("../data/stitching/hill2.jpg")
    img2 = cv2.cvtColor(img2, cv2.COLOR_BGR2GRAY)
    kp1 = sift.detect(img1, None)
    kp2 = sift.detect(img1, None)

    I1 = imread("../data/stitching/hill1.jpg")
    I2 = imread("../data/stitching/hill2.jpg")
    circles1 = [[kp1[i].pt[0], kp1[i].pt[1], kp1[i].size] for i in xrange(len(kp1))]
    circles1 = np.array(circles1)
    circles2 = [[kp2[i].pt[0], kp2[i].pt[1], kp2[i].size] for i in xrange(len(kp2))]
